...
export default function CustomerSignUpPage() {
  ...
  const onSubmit = async (data: CustomerFormData) => {
    ...
    try {
      ...
      if (devSignupBypass) {
        ...
        if (!response.ok) {
          setError(result.error || "We could not create your customer account. Please try again.");
          setLoading(false);
          return;
        }
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          ...
          // Routed through /auth/callback like the other signup forms, so the
          // session actually gets established before landing on the homepage
          // (customers don't get a dashboard, per the team's decision).
      ...
      if (!response.ok) {
        setError(result.error || "We could not save your customer profile. Please try again.");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className={styles.card}>
      ...
      <p className={styles.sub}>Customer sign up</p>
